...
public class TicTacToeSingleFindMistake {
    private char[] board;
    private char player;
    private char computer;

    public TicTacToeSingleFindMistake(char player) {
        board = new char[9];
        this.player = player;
//        computer = (player == 'X') ? 'O' : 'X';
        computer = 'O'; // with my changing
        initializeBoard();
    }
    ...
    public static void main(String[] args) {
        ...
        if (player != 'X' && player != 'O') {
            ...
            TicTacToeSingleFindMistake game = new TicTacToeSingleFindMistake(player);
            game.displayBoard();
            game.playGame();
        }
    }
